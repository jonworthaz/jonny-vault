/* Claude Home — idea → launch core system
...
const STORE_KEY = 'claudehome.ideas.v1';          // shared with Forge (linked mode)
...
function setView(v) {
  ...
  const titles = { dashboard: 'Dashboard', ideas: 'Idea Board', medvi: 'Medvi OS', workflows: 'Workflow Builder', about: 'About Claude Home' };
...
function generateBrief(i) {
  ...
  m += `---\n_Generated by Claude Home on ${fmtDate(now())}._\n`;
  return m;
}
...
function exportData() {
  ...
  const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'claude-home-ideas.json';
  document.body.appendChild(a); a.click(); a.remove(); setTimeout(() => URL.revokeObjectURL(a.href), 1000);
}
